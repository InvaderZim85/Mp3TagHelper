...
# Main entry point
# ===========================
$projectFile = "Mp3TagHelper\Mp3TagHelper.csproj"
...
try {
    ...
    $output = Join-Path -Path $currentLocation -ChildPath "Mp3TagHelper\bin\"
    ...
    # Create the release
    & $dotnet publish "Mp3TagHelper.sln" -p:PublishProfile=Mp3TagHelper/Properties/PublishProfiles/FolderProfile

    # Create the ZIP archive
    $zipDir = Join-Path -Path $output -ChildPath "release\net8.0-windows\publish\win-x64"
    $destFile = Join-Path -Path $output -ChildPath "Mp3TagHelper_v$newVersion.zip"
    Compress-Archive -Path $zipDir -DestinationPath $destFile -CompressionLevel Optimal
